function [imageRegBSpline, imageRegAffine] = fRegBSpline(pointsMRI, pointsHistology, imagesMRI, sizeHistolImageGUI, imageHistology, hist2MRI)
...
%   - imageRegAffine: Registered image with an affine transform. This is
...
%
    ...
    if(hist2MRI)
        ...
        %Not working on its own. Try making an affine first
%         [O_trans, Spacing] = point_registration(sizeHistolImageOri, pointsHistology, pointsMRI, options); % TEST POINTS, IF IT IS (x,y) OR (row,col)
%         imageReg = bspline_transform(O_trans, imageHistology, Spacing, 3);
